...
$(function () {
    (
        function () {
            ...
            bindClick = function () {
                $("#myTags").find("em").off('click').on("click", function () {
            ...
            setTips = function (c, d) {
                ...
                if (isMaxTips()) {
                    layer.msg("最多添加" + G_tocard_maxTips + "个标签！");
                    return false
                }
                var b = d ? 'value="' + d + '"' : "";
...
(function () {
    ...
    // 生成推荐标签
    function initRecommendTag() {
        $.ajax({
            url: '/video/videoTipServer/selectAll',
            success: function (result) {
                $.each(result, function (idx, val) {
                    $("#recommend-tag").append(`<a value = '-1' title = '${val.tipname}' href = "javascript:void(0);" > <span> ${val.tipname} </span><em></em> </a>`);
                });
